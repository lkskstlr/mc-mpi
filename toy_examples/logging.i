...
void init_logging_do_not_call(int world_rank) {
  char filename[100];
  __builtin___sprintf_chk (filename, 0, __builtin_object_size (filename, 2 > 1 ? 1 : 0), "logs/%010d.txt", world_rank);
  loggingFile = fopen(filename, "wb");
}

void stop_logging_do_not_call() { fclose(loggingFile); }
# 2 "logging.c" 2
# 1 "/usr/local/include/mpi.h" 1 3
# 53 "/usr/local/include/mpi.h" 3
...
# 54 "/usr/local/include/mpi.h" 2 3
...
int main(int argc, char const *argv[]) {
  ...
  MPI_Comm_size(((MPI_Comm)0x44000000), &world_size);
  MPI_Comm_rank(((MPI_Comm)0x44000000), &world_rank);

  init_logging_do_not_call(world_rank);
  fprintf(loggingFile, "%s:%d:%s(): " "First Debug print %d", "logging.c", 13, __func__, world_rank);

  stop_logging_do_not_call();
  MPI_Finalize();
  return 0;
}
