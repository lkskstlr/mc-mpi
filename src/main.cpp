...
std::pair<std::string, std::string> parse_input(int argc, char **argv, int world_rank)
{
  ...
  if (argc != 3 || comm_modes.find(argv[2]) == comm_modes.end())
  {
    if (world_rank == 0)
    {
      fprintf(stderr, "Usage: mpirun -n nb_layers %s config_file_path comm_mode\n",
              argv[0]);
...
int main(int argc, char **argv)
{
  ...
  // parse input
  std::pair<std::string, std::string> pair = parse_input(argc, argv, world_rank);
