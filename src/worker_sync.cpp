...
void WorkerSync::spin()
{
    ...
    while (true)
    {
        ...
        timer.change(timestamp, Timer::Tag::Send);
        {
            ...
            //Prepare the particles vector for the extra ones from the left OR right, which are AT MOST options.nb_particles_per_cycle
            old_size = layer.particles.size();
            layer.particles.resize(layer.particles.size() + options.nb_particles_per_cycle);
            /************************** [ODDS] <==> [EVENS] ****************************/
            recv_count = 0;
            if ((world_rank % 2) && (world_rank + 1 < world_size))
            {
                ...
                MPI_Sendrecv(layer.particles_right.data(), layer.particles_right.size(), mpi_particle_type, world_rank + 1, particle_tag,
                             layer.particles.data() + old_size, options.nb_particles_per_cycle, mpi_particle_type, world_rank + 1, particle_tag,
            ...
            layer.particles.resize(old_size + recv_count + options.nb_particles_per_cycle);
            old_size += recv_count;
        ...
        timer.change(timestamp, Timer::Tag::Recv);
        {
            ...
            if (disabled_all == options.nb_particles)
                break;
        }
